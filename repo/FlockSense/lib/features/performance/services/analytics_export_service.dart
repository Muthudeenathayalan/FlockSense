...
class AnalyticsExportService {
        ...
        build: (context) => [
          ...
          pw.Row(
            ...
            children: [
              ...
              pw.Column(
                ...
                children: [
                  pw.Text(
                    'Current Age: ${data.currentAgeDays} Days',
                    style: pw.TextStyle(
                      fontSize: 11,
                      fontWeight: pw.FontWeight.bold,
                    ),
          ...
          pw.Table(
            ...
            children: [
              ...
              _buildTableRow(
                'Mortality %',
                '${data.mortalityPercentage.toStringAsFixed(2)}%',
              ),
              _buildTableRow(
                'Average Weight',
                '${data.avgWeightKg.toStringAsFixed(2)} kg',
              ),
              _buildTableRow(
                'Average Daily Gain (ADG)',
                '${data.avgDailyGainGrams.toStringAsFixed(1)} g/day',
              ),
              _buildTableRow(
                'Feed Consumed',
                '${data.feedConsumedKg.toStringAsFixed(1)} kg',
              ),
              _buildTableRow(
                'Water Consumed',
                '${data.waterConsumedLiters.toStringAsFixed(1)} L',
              ),
              _buildTableRow(
                'Feed Conversion Ratio (FCR)',
                data.fcr.toStringAsFixed(2),
              ),
              _buildTableRow(
                'Medicine Cost',
                _currencyFormat.format(data.medicineCost),
              ),
              _buildTableRow(
                'Total Expenses',
              ...
              _buildTableRow(
                'Estimated Revenue',
                _currencyFormat.format(data.estimatedRevenue),
              ),
              _buildTableRow(
                'Estimated Profit',
                _currencyFormat.format(data.estimatedProfit),
              ),
            ],
          ),
          ...
          if (data.filteredRecords.isEmpty)
            pw.Text(
              'No daily records logged for selected filter range.',
  ...
  static String generateCsvReport(GrowthAnalyticsData data) {
    ...
    sb.writeln(
      'Water Consumed (L),${data.waterConsumedLiters.toStringAsFixed(1)}',
    );
    ...
    sb.writeln(
      'Date,Age (Days),Opening Birds,Mortality,Closing Birds,Feed Consumed (kg),Water Consumed (L),Avg Weight (g)',
    );
  ...
  static Future<void> printOrPreviewPdf(
    ...
    GrowthAnalyticsData data,
  ) async {
    final pdf = await generatePdfReport(data);
    await Printing.layoutPdf(
      onLayout: (PdfPageFormat format) async => pdf.save(),
      name:
          'FlockSense_Growth_Analytics_${DateFormat("yyyyMMdd").format(DateTime.now())}.pdf',
    );
  }
  ...
  static Future<void> shareReport(
    ...
    final file = File(
      '${tempDir.path}/FlockSense_Analytics_${DateFormat("yyyyMMdd_HHmmss").format(DateTime.now())}.pdf',
    );
    ...
    await Share.shareXFiles(
      ...
      text:
          'FlockSense Growth Analytics Report — ${data.activeBatch?.batchName ?? "All Batches"}',
    );
  }
}
