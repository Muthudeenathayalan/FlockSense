...
class ExcelGenerator {
  static Future<Uint8List> generateExcelReport({
    ...
    summarySheet.appendRow([
      ...
      TextCellValue(reportType.title),
    ]);
    summarySheet.appendRow([
      TextCellValue('Farm Name:'),
      TextCellValue(data.farm.farmName),
    ]);
    summarySheet.appendRow([
      TextCellValue('Batch Name:'),
      TextCellValue(data.batch.batchName),
    ]);
    summarySheet.appendRow([
      TextCellValue('Breed:'),
      TextCellValue(data.batch.breedOrFlockType),
    ]);
    summarySheet.appendRow([
      TextCellValue('Generated At:'),
      TextCellValue(DateFormat('yyyy-MM-dd HH:mm').format(data.generatedAt)),
    ]);
    summarySheet.appendRow([TextCellValue('')]);

    summarySheet.appendRow([TextCellValue('METRIC'), TextCellValue('VALUE')]);
    summarySheet.appendRow([
      TextCellValue('Initial Birds'),
      IntCellValue(data.batch.totalBirds),
    ]);
    summarySheet.appendRow([
      TextCellValue('Total Mortality'),
      IntCellValue(data.totalMortality),
    ]);
    summarySheet.appendRow([
      TextCellValue('Livability (%)'),
      DoubleCellValue(double.parse(data.liveabilityPct.toStringAsFixed(2))),
    ]);
    summarySheet.appendRow([
      TextCellValue('Total Feed Consumed (kg)'),
      DoubleCellValue(double.parse(data.totalFeedKg.toStringAsFixed(1))),
    ]);
    summarySheet.appendRow([
      TextCellValue('Total Water Consumed (L)'),
      DoubleCellValue(double.parse(data.totalWaterLiters.toStringAsFixed(1))),
    ]);
    summarySheet.appendRow([
      TextCellValue('Overall FCR'),
      DoubleCellValue(
        double.parse((data.overallFcr ?? 1.55).toStringAsFixed(2)),
      ),
    ]);
    summarySheet.appendRow([
      TextCellValue('Avg Body Weight (g)'),
      DoubleCellValue(
        double.parse((data.avgBodyWeightGrams ?? 0).toStringAsFixed(0)),
      ),
    ]);
    summarySheet.appendRow([
      TextCellValue('Performance Index (EPEF)'),
      DoubleCellValue(double.parse((data.pef ?? 0).toStringAsFixed(1))),
    ]);
    summarySheet.appendRow([
      TextCellValue('Total Revenue'),
      TextCellValue(currencyFormat.format(data.totalRevenue)),
    ]);
    summarySheet.appendRow([
      TextCellValue('Total Expenses'),
      TextCellValue(currencyFormat.format(data.totalExpenses)),
    ]);
    summarySheet.appendRow([
      TextCellValue('Net Profit'),
      TextCellValue(currencyFormat.format(data.netProfit)),
    ]);
    ...
    financeSheet.appendRow([
      ...
      TextCellValue('Bird Sales'),
      DoubleCellValue(data.totalRevenue),
    ]);
    financeSheet.appendRow([
      TextCellValue('Expense'),
      TextCellValue('Feed Cost'),
      DoubleCellValue(data.totalFeedKg * 42.0),
    ]);
    financeSheet.appendRow([
      TextCellValue('Expense'),
      TextCellValue('Chick Purchase'),
      DoubleCellValue(data.batch.totalBirds * 35.0),
    ]);
    financeSheet.appendRow([
      TextCellValue('Expense'),
      TextCellValue('Medicine & Vaccine'),
      DoubleCellValue(
        data.totalExpenses -
