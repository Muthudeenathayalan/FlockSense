...
- 💻 [Local Development Setup](docs/DEVELOPMENT_SETUP.md) — Development environment configuration and emulator setup.
- 🧪 [Testing Guide](docs/TESTING.md) — Test runners, mocking patterns, and widget testing conventions.
- 🤝 [Contributing Guidelines](docs/CONTRIBUTING.md) — Git workflow, branch conventions, and code standards.
- 📋 [Engineering Backlog](docs/ENGINEERING_BACKLOG.md) — Detailed engineering tasks, priorities, and status matrix.
- 📜 [Development Log](docs/DEVELOPMENT_LOG.md) — Chronological log of implemented features, commit hashes, and validation steps.
- 📦 [Changelog](docs/CHANGELOG.md) — Historical release and version milestones.
